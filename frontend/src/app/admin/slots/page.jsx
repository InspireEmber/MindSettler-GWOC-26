    ...
    <div className={`relative bg-white rounded-2xl p-4 md:p-5 border border-slate-100 ${!isModal && "shadow-sm hover:shadow-md"} transition-all flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-6 animate-in slide-in-from-bottom-2`}>
        ...
        <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 lg:flex lg:items-center gap-3 lg:gap-8">
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
                {/* Status Badge */}
                <span className={`px-2.5 py-1 rounded-lg text-[10px] md:text-xs font-bold uppercase tracking-wider border flex items-center gap-1.5 ${
                    s.isBooked ? "bg-slate-100 text-slate-500 border-slate-200" : "bg-emerald-50 text-emerald-700 border-emerald-100"
                }`}>
                    ...
                    {Array.from({ length: getDaysInMonth(currentDate.getFullYear(), currentDate.getMonth()) }).map((_, i) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`h-20 md:h-28 p-1 md:p-2 rounded-lg md:rounded-xl border transition-all relative cursor-pointer group flex flex-col items-center md:items-stretch ${
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl w-full lg:w-auto overflow-x-auto">
                    {["upcoming", "past"].map(t => (
                        <button 
                            ...
                            className={`flex-1 lg:flex-none px-4 md:px-6 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all whitespace-nowrap ${
                                filter.timeline === t ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-700"
                            }`}
                        >
                            {t}
                        </button>
                ...
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar w-full lg:w-auto">
                    {["all", "online", "offline"].map(t => (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase border transition-all whitespace-nowrap ${
                                filter.type === t ? "bg-slate-800 text-white border-slate-800" : "bg-white text-slate-500 border-slate-200 hover:bg-slate-50"
                            }`}
                        >
                            {t}
                        </button>
