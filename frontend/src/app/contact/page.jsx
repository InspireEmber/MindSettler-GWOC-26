    ...
    <div className="min-h-screen bg-white text-[#5E5A6B]">
      ...
      <section className="bg-gradient-to-br from-[#F6F4FA] via-white to-[#F6F4FA] py-20 md:py-32 text-center">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <p className="text-xl leading-relaxed">Have questions? We're here to help. Reach out and we'll get back to you soon.</p>
        </div>
      </section>

      {/* Main Content */}
      <section className="py-16 md:py-24 max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12">
        {/* Sidebar Info */}
        <div className="space-y-8">
          <h2 className="text-3xl font-light text-[#2E2A36]">Contact Information</h2>
          <div className="space-y-6">
            {CONTACT_INFO.map((info, idx) => (
              <div key={idx} className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${info.bgColor}`}>
                  {info.icon}
                </div>
                <div>
                  <h3 className="font-medium text-[#2E2A36]">{info.title}</h3>
                  <a href={info.href} target="_blank" className="hover:text-[#3F2965] transition-colors">{info.value}</a>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="p-8 rounded-2xl bg-[#F6F4FA] border border-[#3F2965]/10">
            <h3 className="text-xl font-medium text-[#2E2A36] mb-4">Response Time</h3>
            <p className="leading-relaxed">Typically 24-48 hours. For urgent booking matters, please mention it in your message.</p>
          </div>
        </div>

        {/* Form */}
        <div className="space-y-8">
          <h2 className="text-3xl font-light text-[#2E2A36]">Send Us a Message</h2>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                required
                rows={5}
                className="w-full px-4 py-3 rounded-xl border border-[#3F2965]/20 focus:ring-2 focus:ring-[#3F2965] outline-none resize-none transition-all"
            ...
            <button
              ...
              className="w-full py-4 rounded-full bg-[#3F2965] text-white font-medium hover:bg-[#3F2965]/90 transition-all hover:shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
            >
