...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F6F4FA] p-2 sm:p-4 relative overflow-hidden">
      ...
      {/* Animated Purple-Pink Rays (same as login) */}
      <motion.div
                ...
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex gap-2">
                  {images.map((_, index) => (
                    <button
                      ...
                      className={`h-2 rounded-full transition-all ${index === currentImageIndex ? "bg-white w-8" : "bg-white/50 w-2 hover:bg-white/70"}`}
        ...
        >
          <div className="flex-1 flex items-center justify-center px-4 sm:px-6 md:px-12 py-4 sm:py-6 md:py-8">
            ...
            >
              ...
              <p className="text-sm sm:text-base text-[#5E5A6B] mb-8">Sign up to book and track your psycho-education sessions more easily.</p>

              {error && (
                <div className="mb-6 rounded-xl bg-red-50 border border-red-100 text-red-700 text-sm px-4 py-3">{error}</div>
              )}
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 flex gap-2">
                  {images.map((_, index) => (
                    <button
                      key={index}
                      onClick={() => goToSlide(index)}
                      className={`h-2 rounded-full transition-all ${index === currentImageIndex ? "bg-white w-8" : "bg-white/50 w-2 active:bg-white/70"}`}
                      aria-label={`Go to slide ${index + 1}`}
                    />
                  ))}
                </div>
