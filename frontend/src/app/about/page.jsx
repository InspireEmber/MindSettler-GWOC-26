    ...
    <div className="min-h-screen bg-white text-[#5E5A6B]">
      ...
      <section className="bg-gradient-to-br from-[#F6F4FA] via-white to-[#F6F4FA] py-20 md:py-32 text-center">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <p className="text-xl leading-relaxed">A compassionate platform dedicated to mental well-being through structured psycho-education.</p>
        </div>
      </section>
      ...
      {[ 
        ...
        <section key={i} className={`py-16 md:py-24 ${s.bg}`}>
          <div className="max-w-6xl mx-auto px-6 md:px-12 grid md:grid-cols-2 gap-12 items-center">
            <div className={s.reverse ? "md:order-2" : ""}>
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="text-sm font-medium uppercase tracking-wider" style={{ color: s.color }}>{s.label}</span>
              </div>
              <h2 className="text-3xl md:text-4xl font-light text-[#2E2A36] mb-6">{s.title}</h2>
              <p className="text-lg leading-relaxed mb-4">MindSettler provides a safe, confidential space to explore thoughts with professional guidance.</p>
              <p className="text-lg leading-relaxed">We strive to foster a culture of empathy, understanding, and proactive mental health care.</p>
            </div>
            <div className={`${s.reverse ? "md:order-1" : ""} h-64 md:h-80 rounded-2xl bg-gradient-to-br from-[#E9E6F2] to-[#F6F4FA] flex flex-col items-center justify-center`}>
              <div className="w-20 h-20 mb-4 rounded-full flex items-center justify-center" style={{ backgroundColor: `${s.color}1A` }}>
                <svg className="w-10 h-10" fill="none" stroke={s.color} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={s.icon} /></svg>
              </div>
              <p className="font-medium">{s.label} Visual</p>
            </div>
          </div>
        </section>
      ))}
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <div className="h-1 w-16 bg-gradient-to-r from-[#3F2965] to-[#DD1764] rounded-full mx-auto mb-6" />
          <h2 className="text-3xl md:text-4xl font-light text-[#2E2A36] mb-12">Our Psycho-Education Approach</h2>
          <div className="space-y-8 text-left">
            {APPROACHES.map((item, i) => (
              <div key={i} className="p-8 rounded-2xl bg-[#F6F4FA] border border-[#3F2965]/10 hover:border-[#3F2965]/30 transition-colors">
                <h3 className="text-2xl font-medium text-[#3F2965] mb-4">{item.title}</h3>
