...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F6F4FA] p-2 sm:p-4 relative overflow-hidden">
      ...
      {/* Animated Purple-Pink Ray */}
      <motion.div
        className="absolute inset-0 -z-0 opacity-20"
        style={{
          background: `linear-gradient(135deg, 
            transparent 0%, 
            rgba(63, 41, 101, 0.15) 25%, 
            rgba(221, 23, 100, 0.15) 50%, 
            rgba(63, 41, 101, 0.15) 75%, 
            transparent 100%)`,
      ...
      {/* Additional subtle ray for depth */}
      <motion.div
        className="absolute inset-0 -z-0 opacity-10"
        style={{
          background: `linear-gradient(45deg, 
            transparent 0%, 
            rgba(221, 23, 100, 0.1) 30%, 
            rgba(63, 41, 101, 0.1) 70%, 
            transparent 100%)`,
      ...
      <div className="w-full max-w-[99vw] xl:max-w-[97vw] 2xl:max-w-[95vw] flex flex-col lg:flex-row gap-4 lg:gap-6 items-stretch justify-center relative z-10 min-h-[600px]">
        ...
        {/* Login Form Unit - First on mobile, Right on desktop */}
        <motion.div
          initial={{ opacity: 0, y: 10, scale: 0.98 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.4, ease: "easeOut" }}
        ...
        >
          ...
          <div className="flex-1 flex items-center justify-center px-4 sm:px-6 md:px-12 py-4 sm:py-6 md:py-8">
            ...
            >
              <h1 className="text-3xl sm:text-4xl font-light text-[#2E2A36] mb-2">
                Welcome Back
              </h1>
              ...
              {error && (
                <div className="mb-6 rounded-xl bg-red-50 border border-red-100 text-red-700 text-sm px-4 py-3">
                  {error}
                </div>
              )}
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input
                      ...
                      className="w-4 h-4 rounded border-[#3F2965]/30 text-[#3F2965] focus:ring-[#3F2965]"
                    />
                  ...
                  <Link
                    href="#"
                    className="text-sm text-[#5E5A6B] hover:text-[#3F2965] transition-colors"
                  >
                    Forgot Password?
                  </Link>
                </div>
              ...
              <p className="text-sm text-[#5E5A6B] text-center">
                ...
                <Link href="/signup" className="text-[#3F2965] font-medium hover:underline">
                  Register
                </Link>
              </p>
            </motion.div>
                  ...
                  {images.map((_, index) => (
                    <button
                      ...
                      className={`h-2 rounded-full transition-all ${
                        index === currentImageIndex
                          ? "bg-white w-8"
                          : "bg-white/50 w-2 active:bg-white/70"
                      }`}
                      aria-label={`Go to slide ${index + 1}`}
                    />
                  ))}
